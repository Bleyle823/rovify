...
export default function TicketsPage() {
    ...
    const { state: web3State, loadEvents: loadWeb3Events, loadUserTickets: loadWeb3UserTickets, listTicketForSale } = useWeb3Events();
    ...
    const loadMarketplaceListings = async () => {
        try {
            ...
            let tm;
            try {
                tm = getContract('TicketMarketplace' as any, activeChainId);
            } catch {
            ...
            const tmAddr = tm.address as `0x${string}`;
            const tmAbi = tm.abi as any;

            const activeListings: number[] = [];
            const listingCards: any[] = [];
            for (const ev of web3State.events || []) {
                try {
                    ...
                    for (const tid of tokenIds) {
                        try {
                            ...
                            if (status === 0) { // Active
                                activeListings.push(Number(tid));
                                listingCards.push({
                                    id: Number(tid),
                                    tokenId: Number(tid),
                                    title: ev.name,
                                    description: ev.description,
                                    price: Number(formatEther((listing.price ?? listing[0]) as bigint)),
                                    currency: 'ETH',
                                    eventId: ev.id,
                                    image: ev.imageUrl,
                                });
            ...
            setListedTokenIds(new Set(activeListings));
            setMarketplaceListings(listingCards);
        } catch {
            setListedTokenIds(new Set());
            setMarketplaceListings([]);
        }
    ...
    useEffect(() => {
        // Initialize from query string (e.g., /tickets?view=nft)
        const qpView = (searchParams?.get('view') || '').toLowerCase();
        if (qpView === 'nft' || qpView === 'upcoming' || qpView === 'past') {
            setActiveTab('mytickets');
            setTicketView(qpView as 'upcoming' | 'past' | 'nft');
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [searchParams]);

    // Also refresh on-chain tickets when wallet/chain changes
    useEffect(() => {
        if (isConnected) {
            loadWeb3Events();
            loadWeb3UserTickets();
    ...
    }, [isConnected, walletAddress, activeChainId, loadWeb3Events, loadWeb3UserTickets]);
    ...
    useEffect(() => {
        const fetchUserTickets = async () => {
            ...
            try {
                ...
                // Load on-chain tickets and events
                await Promise.all([loadWeb3Events(), loadWeb3UserTickets()]);
        ...
        fetchUserTickets();
    }, [isAuthenticated, user, loadWeb3Events, loadWeb3UserTickets]);

    // Map on-chain tickets to UI shape
    const onchainTickets = useMemo(() => {
        const eventsById: Record<number, any> = {};
        (web3State.events || []).forEach((e: any) => { eventsById[Number(e.id)] = e; });

        return (web3State.userTickets || []).map((t: any) => {
            const ev = eventsById[Number(t.eventId)];
            const priceEth = ev ? Number(ev.ticketPrice ?? 0) / 1e18 : 0;
            const startTime = ev ? Number(ev.startTime) : Date.now();
            const location = ev ? (ev.location || 'Unknown Location') : 'On-chain Event';

            const mappedEvent: Event = {
                id: String(t.eventId),
                title: t.eventTitle || (ev?.name || `Event #${t.eventId}`),
                image: ev?.imageUrl || 'https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=600&h=400&fit=crop',
                ...
                category: 'nft',
                hasNftTickets: true,
                price: { currency: 'ETH', min: priceEth, max: priceEth },
                totalTickets: Number(ev?.maxAttendees || 0),
                soldTickets: Number(ev?.ticketsSold || 0),
                organiser: { name: '', image: '', verified: true },
                description: ev?.description || '',
                tags: Array.isArray(ev?.tags) ? ev.tags : [],
            } as unknown as Event;

            const mappedTicket: any = {
                id: String(t.tokenId),
                tokenId: String(t.tokenId),
                event: mappedEvent,
                type: t.ticketType?.toUpperCase?.() || 'GENERAL',
                isNft: true,
                contractAddress: '',
                currency: 'ETH',
                price: priceEth,
                purchaseDate: new Date(Number(t.purchaseTime) || Date.now()).toISOString(),
                status: t.isUsed ? 'USED' : 'ACTIVE',
                transferable: !!t.isTransferable,
                metadata: { perks: [] },
                qrCode: '/vercel.svg',
                seatInfo: {
                    section: t.seatNumber ? `Seat ${t.seatNumber}` : undefined,
                    timeSlot: new Date(Number(t.purchaseTime) || Date.now()).toISOString(),
                },
            };

            return mappedTicket as Ticket & { event: Event };
        });
    }, [web3State.events, web3State.userTickets]);

    // Filter user tickets based on selected view
    const now = new Date();
    const baseFiltered = userTickets.filter(ticket => {
    ...
    const filteredUserTickets = ticketView === 'nft'
        ? onchainTickets.filter(t => !listedTokenIds.has(Number(t.tokenId || t.id)))
        : baseFiltered;

    // Get selected ticket data
    const selectedTicketData = selectedTicket
        ? (ticketView === 'nft'
            ? onchainTickets.find(t => t.id === selectedTicket)
                                    ...
                                    filteredUserTickets.map((ticket) => (
                                        ...
                                        >
                                            ...
                                            <div className="w-1/3 h-32 relative">
                                                <Image
                                                    src={getBestIpfsUrl(ticket.event.image)}
                                ...
                                {selectedTicketData ? (
                                    <div className="bg-white rounded-xl overflow-hidden shadow-neumorph">
                                        ...
                                        <div className="relative h-48">
                                            <Image
                                                src={getBestIpfsUrl(selectedTicketData.event.image)}
                                            ...
                                            <div className="md:col-span-2 space-y-6">
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    {selectedTicketData.transferable && (
                                                        <button
                                                            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-white text-gray-700 hover:bg-gray-50 transition-colors shadow-neumorph-sm hover:shadow-neumorph-inset"
                                                            onClick={async () => {
                                                                const price = prompt('Set listing price in ETH');
                                                                if (!price) return;
                                                                try {
                                                                    await listTicketForSale(Number(selectedTicketData.tokenId || selectedTicketData.id), price);
                                                                    alert('Ticket listed for sale');
                                                                    await loadWeb3UserTickets();
                                                                    await loadMarketplaceListings();
                        ...
                        {isLoading ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {marketplaceListings.map((listing) => (
                                    <div key={listing.id} className="bg-white rounded-xl overflow-hidden shadow-neumorph hover:shadow-lg transition-all transform hover:-translate-y-1 duration-300">
                                        {/* Event Image with Glassmorphism Category */}
                                        <div className="relative h-48">
                                            <Image
                                                src={listing.image || 'https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=600&h=400&fit=crop'}
                                                alt={listing.title}
                                                fill
                                                className="object-cover"
                                            />
                                        ...
                                        <div className="p-4">
                                            ...
                                            <div className="flex items-center gap-4 text-sm mt-1 text-gray-500">
                                                ...
                                                <div className="flex items-center gap-1">
                                                    <FiMapPin className="w-3 h-3 text-[#FF5722]" />
                                                    <span className="truncate max-w-[100px]">On-chain</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex justify-between items-center mt-4">
                                                <div>
                                                    ...
                                                    <p className="font-semibold text-[#FF5722]">ETH {listing.price}</p>
                                                </div>
                                                <button
                                                    onClick={() => alert('Purchase flow not implemented here')}
                            ...
                            <div className="bg-white rounded-xl p-8 text-center shadow-neumorph">
                                ...
                                <h3 className="text-lg font-semibold text-gray-900 mb-1">No marketplace listings</h3>
                                <p className="text-gray-500 mb-6">When you list an NFT ticket, it will appear here.</p>
                                <Link
                                    href="/"
                                    className="px-4 py-2 bg-[#FF5722] text-white rounded-lg shadow-sm hover:bg-[#E64A19] transition-colors"
                                >
